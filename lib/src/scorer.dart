...
class SwipScorer {
  static SwipScore fromProbability({
    ...
    final contributors = <String, double>{};
    for (var i = 0; i < features.length && i < info.inputSchema.length; i++) {
      contributors[info.inputSchema[i]] = features[i];
    }
    return SwipScore(score, contributors, info);
  }
}
